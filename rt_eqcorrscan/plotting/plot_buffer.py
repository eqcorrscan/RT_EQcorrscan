...
from bokeh.models import ColumnDataSource, HoverTool, DataRange1d
...
class EQcorrscanPlot:
    ...
    def __init__(self, rt_client, plot_length, template_catalog, inventory, 
        ...
        self.plot_options = {
            "plot_width": 800, "plot_height": int(800 / len(channels)),
...
def define_plot(doc, rt_client, channels, catalog, inventory, map_options,
    ...
    p1 = figure(
        title=channels[0], 
        x_range=[now - dt.timedelta(seconds=plot_length), now], **plot_options)
    ...
    p1.line(
        x="time", y='data',
        source=trace_sources[channels[0]], color="black",
        line_width=2)
    ...
    if len(channels) > 1:
        for i, channel in enumerate(channels[1:]):
            p = figure(
                title=channel, x_range=p1.x_range, **plot_options)
