...
def check_catalog(catalog_all, catalog_geonet):
    ...
    for ev in catalog_all:
        ...
        else: catalog_origins.append(ev)
    ...
    cat_counts=[no_origin_count, no_mag_count, geonet_no_origin_count, geonet_no_mag_count]
...
def mainshock_mags(mainshock, RT_mainshock):
    ...
    try:
        geonet_mainshock_mag = round(
            (mainshock.preferred_magnitude()
             or mainshock.magnitudes[-1]).mag, 2)
    except IndexError:
    ...
    try:
        geonet_mainshock_mag_uncertainty = round(
            (mainshock.preferred_magnitude()
             or mainshock.magnitudes[-1]).mag_errors.uncertainty,2)
    except IndexError:
        geonet_mainshock_mag_uncertainty = 0.0

    try:
        geonet_mainshock_depth = round(
            (mainshock.preferred_origin()
             or mainshock.origins[-1]).depth/1000, 1)
    except IndexError:
    ...
    try:
        geonet_mainshock_depth_uncertainty = round(
            (mainshock.preferred_origin()
             or mainshock.origins[-1]).depth_errors.uncertainty/1000, 1)
    except IndexError:
        geonet_mainshock_depth_uncertainty = 0.0

    try:    
        RT_mainshock_depth = round(
            (RT_mainshock.preferred_origin()
             or RT_mainshock.origins[-1]).depth/1000, 1)
    except IndexError:
    ...
    try:    
        RT_mainshock_depth_uncertainty = round(
            (RT_mainshock.preferred_origin()
             or RT_mainshock.origins[-1]).depth_errors.uncertainty/1000, 1)
    except IndexError:
        RT_mainshock_depth_uncertainty = geonet_mainshock_depth_uncertainty

    return geonet_mainshock_mag, geonet_mainshock_mag_uncertainty, geonet_mainshock_depth, geonet_mainshock_depth_uncertainty, RT_mainshock_depth, RT_mainshock_depth_uncertainty
...
def _eq_map_test(
    ...
    if topo_res is (None or False):
        ...
        min_region_dim = min(map_region[1] - map_region[0], map_region[3] - map_region[2])
        if min_region_dim > 10:
            topo_res = "01d"
        elif min_region_dim > 2:
            topo_res = "01m"
    ...
    inset_region = [
        ...
        max(-90, map_region[3] + (map_region[3] - map_region[2]) * inset_multiplier/3),
    ]
...
#
 ...
 #
    ...
    with fig.subplot(nrows=1, ncols=1, figsize=("15c", "10c")):
        fig.basemap(
            region=[0, 12, -2, 13], projection="X?", frame="tblr", panel=[0, 0]
        )
        #plot key output information
        fig.text(text="RT-EQcorrscan Aftershock Analysis Outputs", position='TC', offset='j0c/0.4c', font="16p,Helvetica-Bold,black"
        )
    ...
    with fig.subplot(nrows=1, ncols=1, figsize=("15c", "16c")):
        ...
        if colours == 'depth':
            ...
            fig.plot(
                ...
                cmap=True,
                style="cc",
                pen="white", transparency=50, panel=[0,0]
                )
            fig.colorbar(position="jBR+o0.88c/0.8c+h+w4c/0.25c+ml",
        ...
        if colours == 'magnitude':   
            ...
            for i, mag in enumerate(mags_round_no):
                if mag >= magcut:
                    ...
                    mags_new.append(mag)
            mags_new=np.array(mags_new) #would be good to sort all lists, so big mags plot on bottom
            fig.plot(
                x=lons_new,
                y=lats_new,
            ...
            mags_new_o=np.array(mags_new_o) #would be good to sort all lists, so big mags plot on bottom
            fig.plot(
                x=lons_new,
                y=lats_new,
            ...
            fig.colorbar(position="jBR+o0.4c/0.8c+h+w4c/0.25c+ml",
                         frame=["xa1f1+lMagnitude"],
                         )
            fig.text(text='Showing ' + str(len(mags_new)+len(mags_new_o)) + " earthquakes M" + str(magcut) + '+', 
                     position='TR', offset='j0.5c/0.5c', font="14p,Helvetica,black"
                     )  
        ...
        fig.plot(
            ...
            style=f"a1c", 
            color="orange", 
            pen="black", label=f"Relocated mainshock+S0.3c"
            )
        #plot mainshock
        mainshock_origin = mainshock.preferred_origin() or mainshock.origins[-1]
        fig.plot(
            ...
            style=f"a0.6c", 
            color="gold", 
            pen="black", label=f"GeoNet mainshock+S0.3c"
            )
        # Plot stations
        if len(station_lons) and len(station_lats):
            fig.plot(
                ...
                style="i0.5c",
                color="royalblue",
                pen="black", label=f"Seismograph+S0.3c"
            )
        #plot legend
        fig.legend(region=map_region, position="JTL+jTL+o1c")
        
        #plot corners of ellipse
        fig.plot(x=[corners[0][0], corners[1][0], corners[2][0], corners[3][0], corners[0][0]],
                 y=[corners[0][1], corners[1][1], corners[2][1], corners[3][1], corners[0][1]],
                 pen="3p,red", panel=[0,0])
                     
        
        #plotting basemap again to get scale bar on top of plot 
        fig.basemap(
            region=map_region, projection=f"M?", frame=["af", "wsNE"],    
            map_scale="jTR+w10k+o0.5c/0.5c+f"
        )
        
        #plot inset map
        with fig.inset(position=f"jBL+w{inset_width}c+o0c/-0.1c+jBL", projection=inset_proj, box="+p1,black"):
            fig.basemap(region=inset_region, projection=inset_proj, 
                        frame=["af", "wsne"],    
                       )
            fig.coast(
                region=inset_region,
            ...
            fig.plot(data=rectangle, style="r+s", pen="1.5p,black", projection=inset_proj
            )
...
def output_aftershock_map(
    ...
    lats_o = np.array([(ev.preferred_origin() or ev.origins[-1]).latitude for ev in outlier_catalog])
    lons_o = np.array([(ev.preferred_origin() or ev.origins[-1]).longitude for ev in outlier_catalog])
    depths_o = np.array([(ev.preferred_origin() or ev.origins[-1]).depth / 1000. for ev in outlier_catalog])
    
    try:
        mags = np.array([(ev.preferred_magnitude() or ev.magnitudes[-1]).mag for ev in catalog])
        ref_mags=np.array([(ev.preferred_magnitude() or ev.magnitudes[-1]).mag for ev in reference_catalog])
    except:
        mags = np.array([2.0 for ev in catalog])
    ...
    try:
        mags_o = np.array([(ev.preferred_magnitude() or ev.magnitudes[-1]).mag for ev in outlier_catalog])
    except:
        mags_o = np.array([2.0 for ev in outlier_catalog])
    times_o = np.array([(ev.preferred_origin() or ev.origins[-1]).time.datetime for ev in outlier_catalog])
...
def extract_xy(catalog, mainshock):
    ...
    y=[]
    for ev in catalog:
        fwd_azimuth,back_azimuth,distance = GEODESIC.inv(ev.origins[-1].longitude, ev.origins[-1].latitude, mainshock.preferred_origin().longitude, mainshock.preferred_origin().latitude)
...
def extract_xy_csv(lats, lons, mainshock):
    ...
    for i, l in enumerate(lats):
        fwd_azimuth,back_azimuth,distance = GEODESIC.inv(lons[i], l, mainshock.preferred_origin().longitude, mainshock.preferred_origin().latitude)
        if back_azimuth < 0: #put into 0-360 range
            # TODO: Why is this unused?
           back_azimuth = 360 + back_azimuth
        if fwd_azimuth < 0:
...
def get_len_LOWESS(x, y, frac):
    ...
    for i, j in enumerate(x):
        if j > x_95_lims[0] and j < x_95_lims[1] and y[i] > y_95_lims[0] and y[i] < y_95_lims[1]:
            x_95.append(j)
            y_95.append(y[i])
...
def plot_confidence_ellipsoid(catalog, x, y, xo, yo, xrm, yrm, sd, mainshock, radius_km, t, LOWESS, frac):
    ...
    circle=plt.Circle((0,0), radius_km, linestyle='dotted', edgecolor='gray', fc='None', label=r"Search area")
    ...
    ax_nstd.scatter(xrm, yrm, s=100.0, color='orange', marker='*', label='Relocated Mainshock')
    if LOWESS == True and len(x) > 20:
        ax_nstd.plot(smoothed[:,0], smoothed[:,1], c='k', label='Lowess')
    get_cov_ellipse(cov, (x_mean, y_mean), nstd=sd, ax=ax_nstd, label=r'$2\sigma$ ellipse', edgecolor='tomato', linestyle='-', fc='none')
    if len(xo) > 0:
    ...
    #plot line
    ax_nstd.plot([x_start, x_end], [y_start, y_end], linestyle='-', color='gray', label='cross-section')
    

    ax_nstd.set_title('Time since trigger = ' + str(t).split('.')[0] +' (days, HH:MM:SS)' + '\n' +'$2\sigma$ length = ' + str(round(length, 1)) + ' km,    azimuth = ' + str(int(azimuth)).zfill(3) + '$\degree$/ ' + str(int(azimuth+180)).zfill(3) + '$\degree$')
    ax_nstd.legend()
...
def to_xr_yr_mainshock(catalog, mainshock, rotation):
    ...
    x1, y1 = [], []
    mean_lat = math.radians((mainshock.preferred_origin() or mainshock.origins[-1]).latitude)
    for ev in catalog:
        # Degrees east
        xr = math.radians(
...
def plot_confidence_ellipsoid_vertical(catalog, x, z, xo, yo, xrm, zrm, z_m, t, sd, mainshock, w, LOWESS, frac):
    ...
    x_95, z_95= [], []
    for i, j in enumerate(x):
        if j > x_95_lims[0] and j < x_95_lims[1] and z[i] > z_95_lims[0] and z[i] < z_95_lims[1]:
            x_95.append(j)
            z_95.append(z[i])
    ...
    ax_nstd.scatter(xrm, zrm, s=100.0, color='orange', marker='*', label='Relocated_Mainshock')
    ax_nstd.scatter(x_mean, y_mean, s=20.0, color='k', label='Aftershock centroid')
    ...
    ax_nstd.set_title('Time since trigger = ' + str(t).split('.')[0] +' (days, HH:MM:SS)' + '\n' +'$2\sigma$ length (width) = ' + str(round(length, 1)) + ' km,   Dip = ' + str(int(90-azimuth)).zfill(3) + '$\degree$')
    #ax_nstd.legend()
...
def ellipse_plots(catalog_origins, mainshock, relocated_mainshock, fabric_angle, 
    ...
    z_rm=relocated_mainshock[0].origins[-1].depth/1000

    # Identifying outliers
    # rotate into new fabric aligned coordinate system
    x1, y1 = to_xr_yr_mainshock(catalog=catalog_origins, mainshock=mainshock, rotation=fabric_angle)
    #calculate outliers
    ...
    x_z, y_z, z_z=to_xz_yz_z_centroid(catalog=catalog_good_depths, mainshock=mainshock, azimuth=azimuth)
    x_zrm, y_zrm, z_zrm=to_xz_yz_z_centroid(catalog=relocated_mainshock, mainshock=mainshock, azimuth=azimuth)
...
def plot_scaled_magnitudes(mag_list, scaled_mag, slip_list, ref_list, Mw, mainshock):
    ...
    mapping={'Thingbaijam et al. (2017)' : 'o', 
    ...
    #plot magnitudes
    try:
        plt.fill_between(slip_list, mainshock.preferred_magnitude().mag-mainshock.preferred_magnitude().mag_errors.uncertainty, mainshock.preferred_magnitude().mag+mainshock.preferred_magnitude().mag_errors.uncertainty, alpha=0.4, color='lightpink', label='Preferred M unc')
    except:
        x=1
    for i in range(len(slip_list)):
        plt.scatter(slip_list[i], mag_list[i], marker=mapping[ref_list[i]], color=colors[slip_list[i]], label=ref_list[i])  
    ...
    for i, m in enumerate(mainshock.magnitudes):
        if m.magnitude_type in ['MLv', 'ML']:
            plt.axhline(m.mag, color=mag_cols[i], linestyle='solid', label='GeoNet ' + m.magnitude_type)
    plt.axhline(mainshock.preferred_magnitude().mag, color='black', linestyle='solid', label='GeoNet Preferred M')
    ### add labels to top left corner
    try:
        plt.text(0, max_mag-0.1, 'Preferred GeoNet Mag (' + mainshock.preferred_magnitude().magnitude_type + ') = ' +str(round(mainshock.preferred_magnitude().mag, 2)) + 
                 ' (+/- ' + str(round(mainshock.preferred_magnitude().mag_errors.uncertainty,2)) + ')', color='black', fontsize=11.0, horizontalalignment='left')
    except:
        plt.text(0, max_mag-0.1, 'Preferred GeoNet Mag = N/A', color='black', fontsize=11.0, horizontalalignment='left')
    plt.text(0, max_mag-0.2, 'Preferred Scaled Mag (Mas) = ' + str(scaled_mag), color='black', 
             fontsize=11.0, horizontalalignment='left')
    if Mw > 0:
        plt.text(0, max_mag-0.3, 'Moment Magnitude (Mw) = ' + str(Mw), color='black', fontsize=11.0, horizontalalignment='left')
    else: 
...
def summary_files(
    ...
    field_names = ['Trigger_ID', 'Timestamp', 'Elapsed_secs',
                   'N_evs', 'No_origin', 'No_mag', 
                   'N_geonet_evs', 'Geonet_no_origin', 'Geonet_no_mag',
                   'Length', 'Azimuth', 'Dip', 'Width', 
                   'Scaled_mag', 'GeoNet_mag', 'GeoNet_mag_unc',
                   'Mean_depth', 'Relocated_depth', 'Relocated_depth_unc',
                   'GeoNet_ms_depth', 'GeoNet_ms_depth_unc']
     
    # Dictionary that we want to add as a new row
    output_dictionary = {'Trigger_ID': eventid,
    ...
    f_output=output_dir + '/output_metrics_summary_file.csv'  ### NOTE: THIS HAS TO STAY IN LATEST FOLDER
    ...
    f_output_t=output_dir + '/output_metrics_file_' + str(current_time).split('.')[0] + '.csv'
    ...
    with open(".plotting_text.txt", "w") as f:
        ...
        f.write('5.5 10 0 '+ font_size + 'p,Helvetica,black ML ' + str(current_time).split('.')[0] + ' (UTC)' + '\n')
        f.write('0.2 9 0 '+ font_size + 'p,Helvetica-Bold,black ML Elapsed seconds: \n')
        f.write('5.5 9 0 '+ font_size + 'p,Helvetica,black ML ' + str(elapsed_secs) + '\n')
        f.write('0.2 8 0 '+ font_size + 'p,Helvetica-Bold,black ML Total events: \n')
        f.write('5.5 8 0 '+ font_size + 'p,Helvetica,black ML ' + str(len(catalog_RT)) + ' (' + str(len(catalog_outliers)) + ' outliers)\n')
        f.write('0.2 7 0 '+ font_size + 'p,Helvetica-Bold,black ML Fault Length: \n') 
        f.write('5.5 7 0 '+ font_size + 'p,Helvetica,black ML ' + str(round(length, 1)) + ' km' + '\n') 
        f.write('0.2 6 0 '+ font_size + 'p,Helvetica-Bold,black ML Fault Strike/Dip: \n')
        f.write('5.5 6 0 '+ font_size + 'p,Helvetica,black ML ' + str(int(azimuth)).zfill(3) + '/' + str(int(dip)).zfill(2) + ' degrees' + '\n')
        f.write('0.2 5 0 '+ font_size + 'p,Helvetica-Bold,black ML Fault Width: \n')   
        f.write('5.5 5 0 '+ font_size + 'p,Helvetica,black ML ' + str(round(length_z, 1)) + ' km' + '\n')
        f.write('0.2 4 0 '+ font_size + 'p,Helvetica-Bold,black ML Scaled magnitude: \n') 
        f.write('5.5 4 0 '+ font_size + 'p,Helvetica,black ML ' + str(scaled_mag) + '\n')
        f.write('0.2 3 0 '+ font_size + 'p,Helvetica-Bold,black ML GeoNet magnitude: \n') 
        f.write('5.5 3 0 '+ font_size + 'p,Helvetica,black ML ' +  str(geonet_mainshock_mag)+ ' (+/- ' + str(geonet_mainshock_mag_uncertainty) + ')\n')   
        f.write('0.2 2 0 '+ font_size + 'p,Helvetica-Bold,black ML Mean aftershock depth: \n') 
        f.write('5.5 2 0 '+ font_size + 'p,Helvetica,black ML ' + str(mean_depth) + ' km \n')
        f.write('0.2 1 0 '+ font_size + 'p,Helvetica-Bold,black ML Relocated mainshock depth: \n') 
        f.write('5.5 1 0 '+ font_size + 'p,Helvetica,black ML ' + str(RT_mainshock_depth) + ' (+/- ' + str(RT_mainshock_depth_uncertainty) + ') km \n')
        f.write('0.2 0 0 '+ font_size + 'p,Helvetica-Bold,black ML GeoNet mainshock depth: \n') 
        f.write('5.5 0 0 '+ font_size + 'p,Helvetica,black ML ' + str(geonet_mainshock_depth) + ' (+/- ' + str(geonet_mainshock_depth_uncertainty) + ') km')

    return output_dictionary       
                 
                 
def plot_geometry_with_time(times, events, geonet_events, mean_depths, Relocated_depths, Relocated_depth_uncerts, lengths, azimuths, mags, GeoNet_mags, GeoNet_mags_uncerts, GeoNet_depths, GeoNet_depth_uncerts, log, mainshock,  **kwargs):
    ...
    ax1.set_axis_off()
    plt.text(0.5, 1,'RT-EQcorrscan Aftershock Analysis Outputs', weight='bold', color='black', fontsize=14.0, horizontalalignment='center')
    
    plt.text(0, 0.85,'Trigger_ID: ' , color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.85, str(mainshock.resource_id).split('/')[-1], color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.75,'Time since trigger: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.75, str(times[-1]) + ' seconds', color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.65,'Total events: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.65, str(events[-1]), color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.55,'Fault Length: ' , color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.55, str(lengths[-1]) + ' km', color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.44,'Fault Azimuth: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.44,str(azimuths[-1]) + ' degrees', color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.35,'Scaled magnitude: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.35, str(mags[-1]), color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.25,'GeoNet magnitude: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.25, str(GeoNet_mags[-1]), color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.15,'Mean aftershock depth: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.15, str(mean_depths[-1]) + ' km', color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.05,'RT/GN mainshock depth: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.05, str(round(Relocated_depths[-1],1)) + '/' + str(round(GeoNet_depths[-1],1)) + ' km', color='black', fontsize=12.0, horizontalalignment='left')
    ...
    if log==True:
        ax1.set_xscale('log')
    ax1.plot(times, events, linestyle='-', marker='.', markersize='0.01', linewidth=3, label='N events (RT-EQcorrscan)', color='firebrick')
    ax1.plot(times, geonet_events, linestyle='-', marker='.', markersize='0.01', linewidth=3, label='N events (GeoNet)', color='darkgray')
    ...
    plt.axvline(x=3600, color='r', linestyle='dashed')
    plt.axvline(x=86400, color='r', linestyle='dotted')
    ax1.plot(times, mean_depths, linestyle='-', marker='.', markersize='0.01', linewidth=3, label='Mean aftershock depths', color='blueviolet')
    ax1.plot(times, Relocated_depths, linestyle='-', marker='.', markersize='0.01', linewidth=3, label='Relocated mainshock depth', color='plum')
    ...
    ax1.plot(times, GeoNet_depths, linestyle='-', marker='.', markersize='0.01', label='GeoNet preferred mainshock depth', color='darkgray')
    ...
    ax1.fill_between(times, geonet_min_depths, geonet_max_depths, alpha=0.2, color='grey')
    plt.legend()
    plt.ylabel('Depth (km)')
    plt.xlim(10, (86400*7))
    ...
    ax3.plot(times, lengths, linestyle='-', marker='.', markersize='0.01', linewidth=3, label='2$\sigma$ Length', color='green')
    plt.axvline(x=60, color='r', linestyle='-')
    plt.axvline(x=360, color='r', linestyle='-.')
    plt.axvline(x=3600, color='r', linestyle='dashed')
    plt.axvline(x=86400, color='r', linestyle='dotted')
    ...
    ax2.plot(times, azimuths, linestyle='-', marker='.', markersize='0.01', linewidth=3, label='Azimuths', color='orange')
    plt.axvline(x=60, color='r', linestyle='-')
    plt.axvline(x=360, color='r', linestyle='-.')
    plt.axvline(x=3600, color='r', linestyle='dashed')
    plt.axvline(x=86400, color='r', linestyle='dotted')   
    ...
    plt.axvline(x=3600, color='r', linestyle='dashed')
    plt.axvline(x=86400, color='r', linestyle='dotted')
    ax4.plot(times, mags, linestyle='-', marker='.', markersize='0.01', linewidth=3, label='Scaled magnitude', color='blue')
    ax4.plot(times, GeoNet_mags, linestyle='-', marker='.', markersize='0.01', label='GeoNet preferred magnitude', color='darkgray')
...
def plot_geometry_with_time_videos(times, events, geonet_events, mean_depths, dips, Relocated_depths, Relocated_depth_uncerts, lengths, azimuths, mags, GeoNet_mags, GeoNet_mags_uncerts, GeoNet_depths, GeoNet_depth_uncerts, log, mainshock, **kwargs):
    ...
    plt.text(0, 0.75,'Time since trigger: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.75, str(round(times[-1])) + ' seconds', color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.65,'Total events: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.65, str(events[-1]), color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.55,'Fault Length: ' , color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.55, str(round(lengths[-1],1)) + ' km', color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.44,'Fault Strike/Dip: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.44,str(round(azimuths[-1],1)) +'/'+ str(round(dips[-1],1)) + ' degrees', color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.35,'Scaled magnitude: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.35, str(round(mags[-1],2)), color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.25,'GeoNet magnitude: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.25, str(round(GeoNet_mags[-1],2)), color='black', fontsize=12.0, horizontalalignment='left')
    
    plt.text(0, 0.15,'Mean aftershock depth: ', color='black', fontsize=12.0, weight='bold', horizontalalignment='left')
    plt.text(0.45, 0.15, str(round(mean_depths[-1],1)) + ' km', color='black', fontsize=12.0, horizontalalignment='left')
    ...
    if log==True:
        ax1.set_xscale('log')
    ax1.plot(times, events, linestyle='-', marker='.', markersize='0.01', linewidth=3, label='N events (RT-EQcorrscan)', color='#ff9955ff')
    ax1.title.set_text('Events')
    plt.axvline(x=60, color='r', linestyle='-')
    plt.axvline(x=360, color='r', linestyle='-.')
    plt.axvline(x=3600, color='r', linestyle='dashed')
    ...
    plt.axvline(x=3600, color='r', linestyle='dashed')
    plt.axvline(x=86400, color='r', linestyle='dotted')
    ax1.plot(times, dips, linestyle='-', marker='.', markersize='0.01', linewidth=3, label='Dip', color='#ff9955ff')
    plt.legend()
    ...
    if log==True:
        ax3.set_xscale('log')
    ax3.title.set_text('Length')
    ax3.plot(times, lengths, linestyle='-', marker='.', markersize='0.01', linewidth=3, label='2$\sigma$ Length', color='#ff9955ff')
    plt.axvline(x=60, color='r', linestyle='-')
    plt.axvline(x=360, color='r', linestyle='-.')
    plt.axvline(x=3600, color='r', linestyle='dashed')
    plt.axvline(x=86400, color='r', linestyle='dotted')
    ...
    ax2.plot(times, azimuths, linestyle='-', marker='.', markersize='0.01', linewidth=3, label='Strike', color='#ff9955ff')
    plt.axvline(x=60, color='r', linestyle='-')
    plt.axvline(x=360, color='r', linestyle='-.')
    plt.axvline(x=3600, color='r', linestyle='dashed')
    plt.axvline(x=86400, color='r', linestyle='dotted')   
    ...
    plt.axvline(x=3600, color='r', linestyle='dashed')
    plt.axvline(x=86400, color='r', linestyle='dotted')
    ax4.plot(times, mags, linestyle='-', marker='.', markersize='0.01', linewidth=3, label='Scaled magnitude', color='#ff9955ff')
    ax4.plot(times, GeoNet_mags, linestyle='-', marker='.', markersize='0.01', label='GeoNet preferred magnitude', color='darkgray')
    # plot geonet magnitudes and uncertainties
    geonet_max_mags, geonet_min_mags = [],[]
    for i, m in enumerate(GeoNet_mags):
